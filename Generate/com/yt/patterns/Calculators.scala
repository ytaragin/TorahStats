...
object Calculators {
  ...
  def doubleToArray(
      func: (Iterator[Pasuk]) => Double
  )(it: Iterator[Pasuk]): Array[Double] = {
    return Array(func(it))
  }

  def oneValFunc(
      onePFunc: (Pasuk, Double) => Double
  )(it: Iterator[Pasuk]): Array[Double] = {
  ...
  def patternMatcher(
      patt: Array[Trops.Value]
  )(p: Pasuk, curVal: Double): Double = {
  ...
  def patternCount(
      patt: Array[Trops.Value]
  ): Iterator[Pasuk] => Array[Double] = {
    return freqFunc(patternMatcher(patt) _) _
  }

  def calculators(
      td: TorahData
  ): Array[(Array[String], Iterator[Pasuk] => Array[Double])] = {

    var singles =
      ArrayBuffer[(Array[String], Iterator[Pasuk] => Array[Double])]();

    singles.append((Array("Pesukim"), oneValFunc(pasukCount) _))
    singles.append((Array("Words", "WordsPerPasuk"), freqFunc(wordCount) _))
    singles.append(
      (Array("Letters", "LettersPerPasuk"), freqFunc(letterCount) _)
    )
    ...
    singles.append(
      (
        Array("ChiSquare"),
        doubleToArray(NGram.chiSquareGenerator(globalNG, 0) _) _
      )
    )
    ...
    (2 to 8).foreach { dim =>
      singles.append(
        (
          Array("ChiSquare[" + dim + "]"),
          doubleToArray(NGram.chiSquareGenerator(globalNG, dim) _) _
        )
      )
//      singles.append((Array("GTest["+dim+"]"), doubleToArray(NGram.gTestGenerator(globalNG, dim) _) _))
    ...
    Trops.values.foreach { t =>
      singles.append(
        (Array(t.toString, t.toString + " Freq"), patternCount(Array(t)))
      )
    }

    val calcs = Array(
      (
        Array("MerchaTipcha", "MT Freq"),
        patternCount(Array(Trops.MERCHA, Trops.TIPHA))
      ),
      (
        Array("KadmaAzla", "KA Freq"),
        patternCount(Array(Trops.KADMA, Trops.GERESH))
      ),
      (
        Array("KadmaAzlaRevii", "KAR Freq"),
        patternCount(Array(Trops.KADMA, Trops.GERESH, Trops.REVIA))
      ),
      (
        Array("KadmaAzlaMunachRevii", "KAMR Freq"),
        patternCount(
          Array(Trops.KADMA, Trops.GERESH, Trops.MUNACH, Trops.REVIA)
        )
      ),
      (
        Array("DargaTvir", "DT Freq"),
        patternCount(Array(Trops.DARGA, Trops.TEVIR))
      ),
      (
        Array("DargaMunchRevii", "DMR Freq"),
        patternCount(Array(Trops.DARGA, Trops.MUNACH, Trops.REVIA))
      ),
      (
        Array("MunachMunach", "MM Freq"),
        patternCount(Array(Trops.MUNACH, Trops.MUNACH))
      ),
      (
        Array("KadmaMahpachPashta", "KMP Freq"),
        patternCount(Array(Trops.KADMA, Trops.MAHAPACH, Trops.PASHTA))
      ),
      (
        Array("TvirTvir", "TT Freq"),
        patternCount(Array(Trops.TEVIR, Trops.TEVIR))
      )
    )
